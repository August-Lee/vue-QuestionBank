...
new Vue({
    ...
    methods:{
        ...
        getOne:function(){
            axios.get('http://bl.mxmba.com/index.php?c=question_moni_ctrl&m=answer_sheet&shijuan='+this.shijuan+'&time='+this.time)
                .then(response=>{
        ...
        clickRow:function (row, column, cell, event) {
            ...
            location.href="simulationParsing.html?shijuan="+this.shijuan+"&timuId="+row.timu_id;
        },
        goparse:function () {
            console.log(this.timu)
            location.href="simulationParsing.html?shijuan="+this.shijuan+"&timuId="+this.timu;
        },
        handleClose(done) {
            this.$confirm('确认关闭？')
                .then(_ => {
