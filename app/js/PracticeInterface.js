...
new Vue({
    ...
    methods:{
        ...
        radioChange:function(val) {
            ...
            this.isShow=true;
            this.remark="取消查看";
            var link=Number(this.timuId)+1;
            console.log(link);
            if(this.kemu!=null){
                axios.get('http://bl.mxmba.com/index.php?c=tiku_exam_point_ctrl&m=set_answer&id='+this.kemu+'&user_answer='+this.radio1+'&right_answer='+this.all.right_answer+'&question_id='+this.shijuan+'&sub_id='+this.subject_id+'&is_wrong=exam_point')
                    .then(function (response) {
                        location.href="#english"+link;
                        $("#english"+link).text('__'+val+'__');
                        $("#english"+link).css("background","#eee");
                    })
                    .catch(function (response) {
                        console.log(response);
                    });
            }else{
                axios.get('http://bl.mxmba.com/index.php?c=tiku_exam_point_ctrl&m=set_answer&point_id='+this.point_id+'&user_answer='+this.radio1+'&right_answer='+this.all.right_answer+'&question_id='+this.shijuan+'&sub_id='+this.subject_id+'&is_wrong=exam_point')
                    .then(function (response) {
                        location.href="#english"+link;
                        $("#english"+link).text('__'+val+'__');
        ...
        setImg:function(){
            ...
            if(this.fileList2.length==0){
                ...
                axios.get('http://bl.mxmba.com/index.php?c=tiku_exam_point_ctrl&m=set_answer&id=' + this.answerId + '&question_id=' + this.shijuan + '&sub_id=' + this.subject_id + '&question_type=' + this.question_type + '&is_wrong=exam_point',
                    {params}
                )
                    .then(function (response) {
                        // console.log(response);
        ...
        onSubmit:function() {
            var that=this;
            axios.get('http://bl.mxmba.com/index.php?c=tiku_exam_point_ctrl&m=input_error_correction&question_id='+this.shijuan+'&type='+this.typeId+'&content='+this.textarea+'&is_wrong=exam_point')
                .then(function (response) {
                    console.log(response);
        ...
        getOne:function(){
            ...
            if(this.point_id==null){
                axios.get('http://bl.mxmba.com/index.php?c=tiku_exam_point_ctrl&m=show_question&id='+this.kemu+'&complete='+this.timuId)
                    .then(response=>{
                        this.isShow=false;
                        this.remark="答案"
                ...
                axios.get('http://bl.mxmba.com/index.php?c=tiku_exam_point_ctrl&m=show_question&point_id='+this.point_id+'&complete='+this.timuId)
                    .then(response=>{
                        this.isShow=false;
                        this.remark="答案"
                        if(response.data.msg=="没有更多信息"){
